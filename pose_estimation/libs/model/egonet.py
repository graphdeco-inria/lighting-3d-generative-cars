...
from libs.common.img_proc import (
    to_npy,
...
class EgoNet(nn.Module):
    def __init__(self, cfgs, pre_trained=False):
        """
    ...
    def crop_single_instance(
        self, img, bbox, resolution, pth_trans=None, xy_dict=None
    ):
        """
        ...
        if xy_dict is not None and xy_dict["flag"]:
            ...
            instance = np.concatenate(
                [instance, xymap.astype(np.float32)], axis=2
            )
        instance = instance if pth_trans is None else pth_trans(instance)
        return instance

    def crop_instances(
        self, annot_dict, resolution, pth_trans=None, rgb=True, xy_dict=None
    ):
        """
        ...
        for idx, image_tensor in enumerate(annot_dict["image"]):
            ...
            for idx, bbox in enumerate(boxes):
                ...
                ret = modify_bbox(bbox, target_ar)
                c, s, r = ret["c"], ret["s"], 0.0
                instance = image_tensor[None, :, bbox[1] : bbox[3], bbox[0] : bbox[2]]
                instance = self.crop_single_instance(image_tensor.detach().cpu().numpy().transpose(1, 2, 0), bbox, resolution)
                instance = torch.tensor(instance).permute(2, 0, 1)[None, ...].to(image_tensor.device)
    ...
    def write_annot_dict(self, annot_dict, records):
        for idx, path in enumerate(annot_dict["path"]):
            ...
            if "kpts_3d_gt" in annot_dict:
                records[path]["kpts_3d_gt"] = to_npy(
                    annot_dict["kpts_3d_gt"][idx]
                )
            if "pose_vecs_gt" in annot_dict:
                records[path]["pose_vecs_gt"] = to_npy(
                    annot_dict["pose_vecs_gt"][idx]
                )
            if "kpts_3d_before" in annot_dict:
                records[path]["kpts_3d_before"] = to_npy(
                    annot_dict["kpts_3d_before"][idx]
                )
            if "raw_txt_format" in annot_dict:
                # list of annotation dictionary for each instance
                records[path]["raw_txt_format"] = annot_dict["raw_txt_format"][
                    idx
                ]
            if "K" in annot_dict:
                # list of annotation dictionary for each instance
            ...
            if "kpts_3d_gt" in annot_dict and "K" in annot_dict:
                records[path]["arrow"] = self.add_orientation_arrow(
                    records[path]
                )
        return records
    ...
    def get_6d_rep(self, predictions, ax=None, color="black"):
        ...
        for instance_idx in range(len(predictions)):
            ...
            instance_angle, _ = self.kpts_to_euler(template, prediction.T)
    ...
    def gather_lifting_results(
        ...
        record["euler_angles"], record["translation"] = self.get_6d_rep(
            record["kpts_3d_pred"]
        )
        if alpha_mode == "trans":
            record["alphas"] = self.get_observation_angle_trans(
                record["euler_angles"], record["translation"]
            )
        elif alpha_mode == "proj":
            record["alphas"] = self.get_observation_angle_proj(
                record["euler_angles"], record["kpts_2d_pred"], record["K"]
            )
        else:
        ...
        if visualize:
            record = vego.plot_3d_objects(
                prediction, target, pose_vecs_gt, record, color
            )
        return record
    ...
    def plot_one_image(
        ...
        record,
        visualize=False,
        color_dict={"bbox_2d": "r", "bbox_3d": "r", "kpts": ["rx", "b"]},
        ...
        all_kpts_3d_pred = record["kpts_3d_pred"].reshape(
            len(record["kpts_3d_pred"]), -1
        )
        ...
        record = self.gather_lifting_results(
            record,
            all_kpts_2d,
            all_kpts_3d_pred,
            all_kpts_3d_gt,
    ...
    def post_process(
        ...
        for img_path in records.keys():
            ...
            records[img_path] = self.plot_one_image(
                img_path,
                records[img_path],
                visualize=visualize,
    ...
    def get_keypoints(self, instances, records, is_cuda=True):
        ...
        for instance_idx in range(len(local_coord)):
            ...
            screen_coord = affine_transform_modified(
                local_coord[instance_idx], trans_inv
            )
            records[instance_idx]["kpts_2d_pred"] = screen_coord
        return records
    ...
    def forward(self, annot_dict):
        ...
        """
        all_instances, all_records = self.crop_instances(
            annot_dict,
