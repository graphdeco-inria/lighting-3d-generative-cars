...
from viz import ganspace_widget
from viz import inversion_widget
from viz import envmap_select_widget
from viz import shading_widget
...
class Visualizer(imgui_window.ImguiWindow):
    def __init__(self, capture_dir=None):
        super().__init__(title='Car Machine', window_width=3840, window_height=2160)
        ...
        self.latent_widget_density = latent_widget.LatentWidget(self, 'Density')
        ...
        self.ganspace_widget = ganspace_widget.GANSpaceWidget(self)
        self.inversion_widget = inversion_widget.InversionWidget(self)
        self.envmap_select_widget = envmap_select_widget.EnvmapSelectWidget(self)
        self.shading_widget = shading_widget.ShadingWidget(self)
    ...
    def draw_frame(self):
        ...
        self.latent_widget_density(expanded)
        self.stylemix_widget(expanded)
        self.trunc_noise_widget(expanded)
        expanded, _visible = imgui_utils.collapsing_header('Environment Map', default=True)
        self.envmap_select_widget(expanded)
        expanded, _visible = imgui_utils.collapsing_header('Shading', default=True)
        self.shading_widget(expanded)
        ...
        expanded, _visible = imgui_utils.collapsing_header('GANSpace', default=True)
        self.ganspace_widget(expanded)
        expanded, _visible = imgui_utils.collapsing_header('Inversion', default=True)
        self.inversion_widget(expanded)
...
class AsyncRenderer:
    ...
    def set_args(self, **args):
        ...
        update = True
        if self._cur_args is None:
            self._cur_args = deepcopy(args)
        else:
            update = False
            for key in args.keys():
                if "envmap" not in key:
                    if type(args[key]) == dict:
                        a = (set(args[key].values()) != set(self._cur_args[key].values()))
                    else:
                        a = (args[key] != self._cur_args[key])
                    if type(a) != bool:
                        a = any(a)
                    if a:
                        update = True
...
def main(
    ...
    # List pickles.
    pretrained = [
        'https://repo-sam.inria.fr/fungraph/lighting-3d-generative-cars/pretrained/gancars256-128.pkl',
