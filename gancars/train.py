...
SuperResolutionModules = {512: 'training.superresolution.SuperresolutionHybrid8XDC',
                          256:'training.superresolution.SuperresolutionUnet256',
                          128: 'training.superresolution.SuperresolutionHybrid2X'}
...
def launch_training(c, outdir, dry_run):
    ...
    dataset_name = Path(c.training_set_kwargs.path).name
    desc = f'eg3d-{dataset_name}-gpus{c.num_gpus:d}-batch{c.batch_size:d}-gamma{c.loss_kwargs.r1_gamma:g}'
    ...
    with open(os.path.join(c.run_dir, f'{cur_run_id:05d}_training_options.json'), 'wt') as f:
...
def dict2easydict(dict_in):
    if isinstance(dict_in, dict):
        dict_in = dnnlib.EasyDict(dict_in)
        for key, value in dict_in.items():
            dict_in[key] = dict2easydict(value)
    return dict_in
...
@click.option('--data',         help='Training data', metavar='[ZIP|DIR]',                      type=str, required=True, default=None)
@click.option('--envmaps',      help='Training envmaps', metavar='DIR',                         type=str, required=True, default=None)
@click.option('--gpus',         help='Number of GPUs to use', metavar='INT',                    type=click.IntRange(min=1), required=True)
@click.option('--batch',        help='Total batch size', metavar='INT',                         type=click.IntRange(min=1), required=True)
@click.option('--cfg',          help='Base configuration',                                      type=str, required=False)
@click.option('--gamma',        help='R1 regularization weight', metavar='FLOAT',               type=click.FloatRange(min=0), required=False)
@click.option('--config',       help='Path to JSON file specifying all the configutarion',      type=str, default=None)
@click.option('--hash-commit',  help='Hash of the git commit used to execute the code',         type=str, default=None)

@click.option('--cond',         help='Train conditional model', metavar='BOOL',                 type=bool, default=True, required=False)
...
def main(**kwargs):
    """
    Train a StyleGAN using the techniques described in the paper Alias-Free Generative Adversarial Networks.
    """
    ...
    if opts.config is None:
        ...
        c = init_from_config(opts)

    c.hash = opts.hash_commit
    c.envmaps = opts.envmaps

    # Resume.
    if opts.resume is not None:
        c.resume_pkl = opts.resume
        c.ada_kimg = 100 # Make ADA react faster at the beginning.[](../../../../../../data-types)
        c.ema_rampup = None # Disable EMA rampup.
        c.augment_p = opts.p
        try: 
            c.resume_kimg = int(c.resume_pkl[-10:-4])
        except:
            pass 
    ...
    # Launch.
    launch_training(c=c, outdir=opts.outdir, dry_run=opts.dry_run)


def init_from_config(opts):
    with open(opts.config) as json_file:
        c = json.load(json_file)
        c = dict2easydict(c)

    assert c.G_kwargs.channel_base == c.D_kwargs.channel_base
    assert c.G_kwargs.channel_max == c.D_kwargs.channel_max 
    assert c.image_snapshot_ticks == c.network_snapshot_ticks

    c.training_set_kwargs = init_dataset_kwargs(data=opts.data)
    c.training_set_kwargs.use_labels = opts.cond and c.training_set_kwargs.use_labels
    c.training_set_kwargs.xflip = opts.mirror
    c.training_set_kwargs.random_seed = c.random_seed

    if c.training_set_kwargs.resolution == 512:
        sr_module = 'training.superresolution.SuperresolutionUnet512'
    elif c.training_set_kwargs.resolution == 256:
        sr_module = 'training.superresolution.SuperresolutionUnet256'
    ...
    c.G_kwargs.rendering_kwargs.image_resolution = c.training_set_kwargs.resolution,
    c.G_kwargs.rendering_kwargs.superresolution_module = sr_module
    
    c.num_gpus = opts.gpus
    c.batch_size = opts.batch
    c.ema_kimg = c.batch_size * 10.0 / 32.0
    c.batch_gpu = c.batch_size // c.num_gpus
    c.loss_kwargs.blur_fade_kimg = c.batch_size * 200.0 / 32.0 # Fade out the blur during the first N kimg.
    return c


def init_manual(opts):
