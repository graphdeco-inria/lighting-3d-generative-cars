    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            with imgui_utils.item_width(viz.font_size * 10):
                _changed, self.fov = imgui.slider_float('##fov', self.fov, 12, 90, format='%.2f Degrees')
            ...
            imgui.same_line(viz.label_w)
            with imgui_utils.item_width(viz.font_size * 10):
                _changed, self.radius = imgui.slider_float('##radius', self.radius, -5, 5, format='%.2f')

            imgui.same_line(viz.label_w + viz.font_size * 13 + viz.button_w + viz.spacing * 3)
            snapped = round(self.radius)
            if imgui_utils.button('Snap', width=viz.button_w, enabled=(self.radius != snapped)):
                self.radius = snapped
            imgui.same_line()
            if imgui_utils.button('Reset', width=-1, enabled=(abs(self.radius - self.radius_default)) > .01):
                self.radius = self.radius_default

        viz.args.focal_length = float(1 / (np.tan(self.fov * 3.14159 / 360) * 1.414))
        viz.args.camera_radius = self.radius
#----------------------------------------------------------------------------
