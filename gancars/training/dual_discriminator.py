...
def filtered_resizing(image_orig_tensor, size, f, filter_mode='antialiased'):
    if filter_mode == 'antialiased':
        ada_filtered_64 = torch.nn.functional.interpolate(image_orig_tensor, size=size, mode='bilinear', align_corners=False, antialias=True)
    elif filter_mode == 'classic':
        size[0] = size[0] * 2 + 2
        size[1] = size[1] * 2 + 2
        ada_filtered_64 = upfirdn2d.upsample2d(image_orig_tensor, f, up=2)
        ada_filtered_64 = torch.nn.functional.interpolate(ada_filtered_64, size=size, mode='bilinear', align_corners=False)
        ada_filtered_64 = upfirdn2d.downsample2d(ada_filtered_64, f, down=2, flip_filter=True, padding=-1)
    elif filter_mode == 'none':
        ada_filtered_64 = torch.nn.functional.interpolate(image_orig_tensor, size=size, mode='bilinear', align_corners=False)
    elif type(filter_mode) == float:
        assert 0 < filter_mode < 1

        filtered = torch.nn.functional.interpolate(image_orig_tensor, size=size, mode='bilinear', align_corners=False, antialias=True)
        aliased  = torch.nn.functional.interpolate(image_orig_tensor, size=size, mode='bilinear', align_corners=False, antialias=False)
...
class DualDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, update_emas=False, **block_kwargs):
        image_raw = filtered_resizing(img['lr_image'], size=img['sr_image'].shape[-1], f=self.resample_filter)
        img = torch.cat([img['sr_image'], image_raw], 1)
...
class DummyDualDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, update_emas=False, **block_kwargs):
        ...
        image_raw = filtered_resizing(img['lr_image'], size=img['sr_image'].shape[-1], f=self.resample_filter) * self.raw_fade
        img = torch.cat([img['sr_image'], image_raw], 1)
